...
func setup(controller: Control) -> void:
	...
	button(row,"Pause / resume",_toggle_pause)
	button(row,"Single step",_single_step)
	button(row,"Fresh tower",func() -> void: host.tower_reset())
	button(row,"Tuning",func() -> void: host.tower_tuning())
	button(row,"Water Feel",_water_reset)
	...
	blind_button=button(release,"Blind A/B/C",_start_blind)
	next_blind_button=button(release,"Next blind",_next_blind)
	var h_row: HBoxContainer = HBoxContainer.new()
	add_child(h_row)
	h_rank=OptionButton.new();h_rank.focus_mode=Control.FOCUS_NONE
	h_rank.add_item("Rank —",0);h_rank.add_item("Rank 1",1);h_rank.add_item("Rank 2",2);h_rank.add_item("Rank 3",3)
	h_row.add_child(h_rank)
	...
	h_note=LineEdit.new();h_note.placeholder_text="Short observation / nuance";h_note.custom_minimum_size.x=280
	h_row.add_child(h_note)
	h_capture_button=button(h_row,"Capture H observation",_capture_h)
	h_reveal_button=button(h_row,"Finish / reveal blind",_request_reveal)
	h_open_button=button(h_row,"Open H folder",_open_h_folder)
...
func _ensure_h_session() -> bool:
	...
	h_feedback.text="H session %s started — capture creates screenshot + metadata + breadcrumb" % h_recorder.session_id
	return true
...
func _safe_h_metadata(context: Dictionary) -> Dictionary:
	...
	if label.is_empty(): metadata["effective_policy"]=policy.duplicate(true);metadata["effective_policy_hash"]=str(context.get("water_policy_hash",""))
		...
		var visible: Dictionary=CyberWaterExperimentBlind.visible_record(blind_set,label,{"scenario_id":metadata.scenario_id,"seed":metadata.seed,"recipe_hash":metadata.recipe_hash})
...
func _start_blind() -> void:
	...
	h_blind_revealed=false;h_retained_blind_set.clear();h_recorder.record_event("blind-set-request",{})
	if not host.water_lab_prepare_blind(): h_feedback.text="Blind set was not started; existing controlled run may still be active"
func _next_blind() -> void:
	...
	if not host.water_lab_apply_blind(): h_feedback.text="No active blind set to advance"
...
func refresh(active: bool, floor_index: int, context: Dictionary) -> void:
	...
	if context.get("water_active",false):
		floor_picker.disabled=true;tube_picker.disabled=true;var blind_active: bool=(not str(context.get("water_blind_label","")).is_empty() or not host.water_blind_set.is_empty())
		...
		blind_button.disabled=blind_active;next_blind_button.disabled=not blind_active or h_blind_revealed;h_reveal_button.disabled=not blind_active and h_retained_blind_set.is_empty();water_policy_button.disabled=blind_active;water_save_button.disabled=blind_active;h_capture_button.disabled=false
		for label: Label in labels: label.visible=false
