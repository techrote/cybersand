...
func _test_recorder_files() -> void:
	...
	if sequence.get("ok",false):
		var frame1: Dictionary=recorder.capture_frame_sequence_frame(image,{"tick":124,"render_snapshot_serial":7,"camera":{"x":0.0,"y":0.0},"view":{"width":8,"height":8}})
		var frame2: Dictionary=recorder.capture_frame_sequence_frame(image,{"tick":125,"render_snapshot_serial":8,"camera":{"x":0.0,"y":0.0},"view":{"width":8,"height":8}})
		...
		_expect(FileAccess.file_exists(str(sequence.root)+"/F000002-t00000125.png"),"second frame-sequence PNG missing")
