...
struct CellLayoutStorage {
    ...
    static constexpr unsigned epoch_shift = wide ? 56U : 24U;
    ...
    static constexpr Word epoch_mask = Word{255} << epoch_shift;
    ...
    constexpr void set_epoch(std::uint8_t v) noexcept { bits = (bits & ~epoch_mask) | (static_cast<Word>(v) << epoch_shift); }
