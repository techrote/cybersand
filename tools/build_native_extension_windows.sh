...
mingw_cxx="${WINDOWS_CXX:-${MINGW_CXX:-x86_64-w64-mingw32-g++}}"
expected_llvm_fragment="23.1.0"
expected_library_sha="db7a8c71f32ba9168b380d6a2599352bb2182178797745dd93956e5294638b09"
expected_output_sha="58451b8aad79fbaed7833707c26d3f6c54da07cfb26cd2c59ed6ef87651b2aa9"
...
if ! command -v "$mingw_cxx" >/dev/null 2>&1; then
    ...
    echo "Set WINDOWS_CXX (or legacy MINGW_CXX) to the pinned LLVM-MinGW compiler." >&2
    exit 2
fi

godot_cpp_library="${GODOT_CPP_LIBRARY:-$godot_cpp_root/bin/libgodot-cpp.windows.template_debug.x86_64.a}"
output="${CYBERSAND_NATIVE_OUTPUT:-$project_root/godot/addons/cybersand_native/bin/cybersand_native.windows.x86_64.dll}"
...
compiler_version="$("$mingw_cxx" --version | head -n 1)"
if [[ "$compiler_version" != *"$expected_llvm_fragment"* && "${CYBERSAND_ALLOW_TOOLCHAIN_DRIFT:-0}" != "1" ]]; then
    echo "Windows compiler mismatch: expected LLVM $expected_llvm_fragment" >&2
    echo "actual: $compiler_version" >&2
    exit 3
fi
link_flags=(-Wl,--no-undefined -static)
...
command=(
    ...
    -shared -pthread -Wno-c++11-narrowing
...
printf 'Windows GDExtension build command:'
printf ' %q' "${command[@]}"
printf '\n'
if [[ "${CYBERSAND_DRY_RUN:-0}" == "1" ]]; then
    exit 0
