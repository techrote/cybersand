...
p.add_argument('--smoke',action='store_true');a=p.parse_args();a.output.mkdir(parents=True,exist_ok=True)
exe=ROOT/'build/transport_characterisation.exe'
artifacts=[exe,*a.profiles.glob('*.ints'),*a.profiles.glob('*.json')]
(a.output/'manifest.json').write_text(json.dumps(identity(artifacts),indent=2))
results=[]
for profile in range(3):
    for sampled in ([0] if a.smoke else [0,1]):
        for layout in ['powder','erosion','loose','poured','packed','film']:
            for seed in ([0] if a.smoke else range(5)):
                for workers in ([1] if a.smoke else [1,4]):
                    ...
                    summary=dict(id=key,profile=profile,sampled=sampled,layout=layout,seed=seed,workers=workers,
